function lowpass_filter = gaussian_lowpass_filter(image,high_freq,scale_factor)
    ...
    G = exp(-4*log(2)*(X-center_x).^2/(2*high_freq)^2) .* exp(-4*log(2)*(Y-center_y).^2/(2*high_freq)^2/scale_factor^2);
    ...
    lowpass_filter = ifft2(adjusted_freq_domain);
end